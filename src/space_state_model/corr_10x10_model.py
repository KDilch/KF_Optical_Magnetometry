...
class Corr_10x10_CC_Sensor_Model(Model):
    ...
    def step(self, method="default"):
        ...
        dx_dt_bound = partial(self.fx, params=self._params)
    ...
    def B(x, t, params):
        B = np.zeros(10)
        B[0] = 2 * np.sqrt(params.eta * params.measurement_strength) * x[7]
        B[1] = 2 * np.sqrt(params.eta * params.measurement_strength) * x[5]
        B[2] = 2 * np.sqrt(params.eta * params.measurement_strength) * x[8]
        B[3] = params.num_atoms / 4 * np.sin(x[9] * t) * (np.cos(x[9] * t)) ** 2
        B[4] = -params.num_atoms / 4 * np.sin(x[9] * t) * (np.cos(x[9] * t)) ** 2
        B[5] = 0
        B[6] = -params.num_atoms / 16 * (np.cos(x[9] * t) + np.cos(3 * x[9] * t))
        B[7] = 0
        B[8] = 0
        B[9] = 0
        return B.reshape((10, 1))
