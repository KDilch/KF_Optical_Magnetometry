...
class CC_UKF(object):
    ...
    def __init__(self, model_params):
        ...
        def predict_update(self, dz):
            ...
            dP = np.dot(self.F(self._x, self._t), self._P) * self._dt + np.dot(self._P, np.transpose(
                self.F(self._x))) * self._dt - np.dot(np.dot(self._K, self._H), self._P) * self._dt + self._Q * self._dt
            self._x += dx
            self._P += dP
            self._t += self._dt
            return
