...
class BellBloomEKF(CD_EKF):
    def __init__(self, model_params):
        CD_EKF.__init__(self, model_params=model_params)
    ...
    def dx_dt(t, x, K, y, dt, model_params):
        return BellBloomEKF.fx(x, t, model_params) + np.dot(K, y) / dt

    @staticmethod
    def dP_dt(t, P, x, K, H, Q, dim_x, model_params):
        return np.reshape(np.dot(BellBloomEKF.F(x, t, model_params),
                                 np.reshape(P, (dim_x, dim_x))) + np.dot(np.reshape(P, (dim_x, dim_x)),
                                                                         np.transpose(BellBloomEKF.F(x,
                                                                                                        t,
                                                                                                        model_params))) - np.dot(
    ...
    def predict_update(self, dz, compute_ss=False):
        ...
        P_sol = solve_ivp(BellBloomEKF.dP_dt,
        ...
        x_sol = solve_ivp(BellBloomEKF.dx_dt,
                          [self._t, self._t + self._dt],
                          self._x,
                          method=self.model_params.inference_method,
                          dense_output=True,
