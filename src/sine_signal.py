...
x = A*np.sin(omega_0*t + phi) + np.random.normal(scale=sigma, size=len(t))


# Compute the CRB for frequency
def CRB_f(t):
    CRB_approx = (6*sigma**2)/(A**2*t**(3))
...
error_array = 100*np.ones_like(t)
...
ax2.semilogy(t, error_array_np_avg, label='smoothed')
ax2.semilogy(t, error_array_smoothed, label='golay')
